# EvaEmailVerification Golang SDK Reference

Complete API reference for the EvaEmailVerification Golang SDK.
...
#### `TestSDK(testopts, sdkopts map[string]any) *EvaEmailVerificationSDK`
...
#### `Email(data map[string]any) EvaEmailVerificationEntity`
...
```go
result, err := client.Email(nil).Load(map[string]any{"email": "email"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(result)
