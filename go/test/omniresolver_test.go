...
import (
	...
	omni "github.com/voxgig-sdk/eva-email-verification-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.EvaEmailVerificationError); is {
				out := map[string]any{
					"name":    "EvaEmailVerificationError",
...
func omniCtx(arg any, client *sdk.EvaEmailVerificationSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.EvaEmailVerificationSDK); is {
			client = live
