...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("EVA_EMAIL_VERIFICATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("EVA_EMAIL_VERIFICATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("EVA_EMAIL_VERIFICATION_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.EvaEmailVerificationSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.EvaEmailVerificationSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.EvaEmailVerificationError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.EvaEmailVerificationSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.EvaEmailVerificationError{Msg: msg, Code: code}
}
