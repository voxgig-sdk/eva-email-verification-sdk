...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("EVAEMAILVERIFICATION_TEST_LIVE") == "TRUE" ||
		os.Getenv("EVAEMAILVERIFICATION_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("EVAEMAILVERIFICATION_TEST_EXPLAIN"); explain != "" {
