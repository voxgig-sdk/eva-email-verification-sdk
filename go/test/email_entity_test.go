...
func emailBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("EVA_EMAIL_VERIFICATION_TEST_EMAIL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["EVA_EMAIL_VERIFICATION_TEST_EMAIL_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["EVA_EMAIL_VERIFICATION_TEST_LIVE"] == "TRUE" {
		mergedOpts := vs.Merge([]any{
			map[string]any{
			},
			extra,
		})
		client = sdk.NewEvaEmailVerificationSDK(core.ToMapAny(mergedOpts))
	}
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["EVA_EMAIL_VERIFICATION_TEST_EXPLAIN"] == "TRUE",
		live:          live,
