# eva-email-verification-mcp

[MCP](https://modelcontextprotocol.io) server exposing the EvaEmailVerification SDK as
two agent tools — `eva-email-verification_list` and `eva-email-verification_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/eva-email-verification-mcp)
make build

# 2. Provide credentials via the environment
export EVA_EMAIL_VERIFICATION_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user eva-email-verification \
  -- /absolute/path/to/eva-email-verification-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./eva-email-verification-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// eva-email-verification_list: first page of records
{ "entity": "email" }
{ "entity": "email", "query": { } }

// eva-email-verification_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/eva-email-verification-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user eva-email-verification \
     -- "$PWD"/dist/*/eva-email-verification-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `eva-email-verification_list` and `eva-email-verification_load` tools now appear
   in new sessions. Ask the agent to *"list email using eva-email-verification"*
   and it calls `eva-email-verification_list` with `{"entity":"email"}`.
...
```sh
export EVA_EMAIL_VERIFICATION_APIKEY=sk_live_xxx            # API key
export EVA_EMAIL_VERIFICATION_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `eva-email-verification_list` tool
...
### Call the `eva-email-verification_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `eva-email-verification_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `eva-email-verification_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `EVA_EMAIL_VERIFICATION_APIKEY` | API key sent with every request. |
| `EVA_EMAIL_VERIFICATION_BASE` | Optional override of the API base URL. |
...
```sh
./eva-email-verification-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"eva-email-verification_load","arguments":{"entity":"email","query":{"id":1}}}}'
```
