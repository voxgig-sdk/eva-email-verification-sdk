# eva-email-verification-cli

boru-driven command-line client **and** interactive REPL for the EvaEmailVerification
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/eva-email-verification-cli)
make build

# 2. See usage (words, entities, env vars)
./eva-email-verification-cli --help

# 3. Provide credentials once, via the environment
export EVA_EMAIL_VERIFICATION_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./eva-email-verification-cli load 1 email            # {id:1} shorthand
./eva-email-verification-cli load '{id:1}' email       # explicit match map

# 5. Override the API base URL for a single call
EVA_EMAIL_VERIFICATION_BASE=https://api.example.com ./eva-email-verification-cli load 1 email

# 6. No arguments -> interactive REPL
./eva-email-verification-cli
eva-email-verification> load 1 email
eva-email-verification> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/eva-email-verification-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/eva-email-verification-cli load 1 email
   ```
...
```sh
./eva-email-verification-cli load 1 email          # scalar shorthand for {id:1}
./eva-email-verification-cli load '{id:1}' email     # explicit match map
```
...
./eva-email-verification-cli load 1 email
```
...
Run with no arguments to open a REPL (prompt `eva-email-verification>`). Each line is
evaluated as its own boru expression:

```text
$ ./eva-email-verification-cli
eva-email-verification> load 1 email
eva-email-verification> /help
eva-email-verification> /quit
```
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/eva-email-verification-cli`. |
...
(`./eva-email-verification-cli <expr>`) and interactively (the REPL), and expressions compose the
