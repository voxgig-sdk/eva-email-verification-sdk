# EvaEmailVerification SDK test runner
...
class EvaEmailVerificationTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            EvaEmailVerificationTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = EvaEmailVerificationTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = EvaEmailVerificationTestRunner.getenv("EVA_EMAIL_VERIFICATION_TEST_LIVE")
        override = EvaEmailVerificationTestRunner.getenv("EVA_EMAIL_VERIFICATION_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = EvaEmailVerificationTestRunner.getenv(key)
        ...
        explain = EvaEmailVerificationTestRunner.getenv("EVA_EMAIL_VERIFICATION_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if EvaEmailVerificationTestRunner._test_control is not None:
            return EvaEmailVerificationTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                EvaEmailVerificationTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            EvaEmailVerificationTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = EvaEmailVerificationTestRunner.load_test_control()
...
def load_env_local():
    EvaEmailVerificationTestRunner.load_env_local()


def env_override(m):
    return EvaEmailVerificationTestRunner.env_override(m)


def entity_data(v):
    return EvaEmailVerificationTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return EvaEmailVerificationTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return EvaEmailVerificationTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return EvaEmailVerificationTestRunner.load_test_control()


def live_client_options():
    return EvaEmailVerificationTestRunner.live_client_options()


def live_delay_ms():
    return EvaEmailVerificationTestRunner.live_delay_ms()
