...
class EvaEmailVerificationSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = EvaEmailVerificationHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = EvaEmailVerificationHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!EvaEmailVerificationFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, EvaEmailVerificationFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return EvaEmailVerificationUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = EvaEmailVerificationHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = EvaEmailVerificationHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = EvaEmailVerificationHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new EvaEmailVerificationError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = EvaEmailVerificationHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new EvaEmailVerificationError("graphql_error",
                "EvaEmailVerificationSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new EvaEmailVerificationSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
