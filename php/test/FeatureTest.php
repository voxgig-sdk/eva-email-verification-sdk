...
// EvaEmailVerification SDK feature test
//
...
class FtCtrl extends EvaEmailVerificationControl
{
...
class FtClient
{
    ...
    private EvaEmailVerificationUtility $utility;

    public function __construct(EvaEmailVerificationUtility $utility, array $options)
    {
    ...
    public function get_utility(): EvaEmailVerificationUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (EvaEmailVerificationContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public EvaEmailVerificationContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (EvaEmailVerificationContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = EvaEmailVerificationConfig::make_feature($name);
    ...
    private static function buildUrl(EvaEmailVerificationSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof EvaEmailVerificationError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new EvaEmailVerificationResponse($fetched);
        }
    ...
    private function fail(EvaEmailVerificationContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (EvaEmailVerificationContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
