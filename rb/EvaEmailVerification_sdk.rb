...
require_relative 'EvaEmailVerification_types'
...
class EvaEmailVerificationSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = EvaEmailVerificationHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = EvaEmailVerificationHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, EvaEmailVerificationFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    EvaEmailVerificationUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = EvaEmailVerificationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = EvaEmailVerificationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = EvaEmailVerificationHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => EvaEmailVerificationError.new(
        "#{op}_allow",
        "EvaEmailVerificationSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue EvaEmailVerificationError => err
    ...
    if fetched.is_a?(Hash)
      status = EvaEmailVerificationHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = EvaEmailVerificationError.new(
        "graphql_error", "EvaEmailVerificationSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = EvaEmailVerificationSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
