...
module EvaEmailVerificationUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = EvaEmailVerificationResponse.new({})
    ...
    if fetch_err
      ...
      response = EvaEmailVerificationResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = EvaEmailVerificationResponse.new(fetched)
    else
