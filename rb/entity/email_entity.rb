# EvaEmailVerification SDK Email entity
...
class EmailEntity
  ...
  def data_set(args)
    if args
      @_data = EvaEmailVerificationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = EvaEmailVerificationHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Email, Hash] the loaded Email; raises EvaEmailVerificationError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = EvaEmailVerificationHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
