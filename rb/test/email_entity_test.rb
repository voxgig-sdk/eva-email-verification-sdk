...
class EmailEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set EVA_EMAIL_VERIFICATION_TEST_EMAIL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def email_basic_setup(extra)
  ...
  entid_env_raw = ENV["EVA_EMAIL_VERIFICATION_TEST_EMAIL_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["EVA_EMAIL_VERIFICATION_TEST_EMAIL_ENTID"])
  ...
  if env["EVA_EMAIL_VERIFICATION_TEST_LIVE"] == "TRUE"
    ...
    client = EvaEmailVerificationSDK.new(Helpers.to_map(merged_opts))
  end
