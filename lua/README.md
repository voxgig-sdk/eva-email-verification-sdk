# EvaEmailVerification Lua SDK



The Lua SDK for the EvaEmailVerification API — an entity-oriented client using Lua conventions.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/eva-email-verification-sdk/releases)),
...
```lua
local email, err = client:Email():load({ email = "example_email" })
...
```lua
local email, err = client:Email():load({ email = "example" })
if err then error(err) end
```
...
local result, err = client:Email():load({ email = "example" })
...
```lua
local email, err = client:Email():load({ email = "email" })
```
...
```
lua/
├── eva-email-verification_sdk.lua    -- Main SDK module
...
The main module (`eva-email-verification_sdk`) exports the SDK constructor
