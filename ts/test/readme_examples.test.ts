...
//     (\`new EvaEmailVerificationSDK(...)\` / \`EvaEmailVerificationSDK.test(...)\` become
//     \`EvaEmailVerificationSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'EvaEmailVerificationSDK'
...
const TEST_SEED = {"entity":{"email":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new EvaEmailVerificationSDK(...)\` and any
// \`EvaEmailVerificationSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+EvaEmailVerificationSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/EvaEmailVerificationSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
