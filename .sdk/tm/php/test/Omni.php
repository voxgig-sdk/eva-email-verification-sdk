...
//    only - a typed EvaEmailVerificationContext there would make every ctx
...
final class EvaEmailVerificationOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new EvaEmailVerificationContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new EvaEmailVerificationSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new EvaEmailVerificationResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new EvaEmailVerificationError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new EvaEmailVerificationResponse($clean['response']);
    ...
    public static function err_from_map($m): ?EvaEmailVerificationError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new EvaEmailVerificationError(is_string($code) ? $code : '', $msg);
    }
