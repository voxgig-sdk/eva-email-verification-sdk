...
// EvaEmailVerification SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = EvaEmailVerificationOmni::makeStructRunner(
    ...
    {
        return EvaEmailVerificationOmni::objmodel(EvaEmailVerificationOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return EvaEmailVerificationOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return EvaEmailVerificationOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(EvaEmailVerificationOmni::structfix($now, Struct::undef()))
                        !== json_encode(EvaEmailVerificationOmni::structfix($val, Struct::undef()))) {
