...
// EvaEmailVerification SDK retry feature
...
class EvaEmailVerificationRetryFeature extends EvaEmailVerificationBaseFeature
{
    ...
    public function init(EvaEmailVerificationContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (EvaEmailVerificationContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(EvaEmailVerificationContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(EvaEmailVerificationContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
