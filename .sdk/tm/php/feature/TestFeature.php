...
// EvaEmailVerification SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class EvaEmailVerificationTestFeature extends EvaEmailVerificationBaseFeature
{
    ...
    public function init(EvaEmailVerificationContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (EvaEmailVerificationContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = EvaEmailVerificationParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (EvaEmailVerificationContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(EvaEmailVerificationContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = EvaEmailVerificationParam::call($ctx, $k);
